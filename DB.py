...
class DB:
  def __init__(self, host='localhost', database='redacoes', user='pguser', password='pgpassword'):
    self.host = host
  ...
  def criarTabela(self, tabela="redacao", colunas='id serial primary key, titulo varchar, tema varchar, redacao varchar, nota real[]'):
    # Conectando ao banco
  ...
  def inserirNaTabela(self, tabela="redacao", valores="default,'Problemas no Piauí','O problema foi devido a falta de água'"):
    # Conectando ao banco
    conexao = self.conectar()
    # Caso ocorra erro na conexão com o banco, nem executará o resto da função
    if(not conexao):
  ...
  def listarDadosDaTabela(self, tabela="redacao", condicao="*"):
    # Conectando ao banco
    conexao = self.conectar()
    # Caso ocorra erro na conexão com o banco, nem executará o resto da função
    if(not conexao):
  ...
  def deletar_tabela(self, tabela="redacao"):
    # Conectando ao banco
    conexao = self.conectar()
    # Caso ocorra erro na conexão com o banco, nem executará o resto da função
    if(not conexao):
